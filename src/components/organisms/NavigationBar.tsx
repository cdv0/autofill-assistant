  ...
  return (
    ...
    <div className="flex flex-col bg-white rounded-2xl border border-stroke p-4 gap-2 justify-between min-h-[calc(100vh)] min-w-64 max-w-80">
      {/* TOP SECTION */}
        ...
        {/* BOTTOM SECTION */}
        <div className="flex flex-col gap-2 pt-1">
          <NavItem style="lightGray" icon={CircleUserRound} text="Account" onClick={onClickAccount}/>
          <NavItem style="danger" icon={LogOut} text="Log out" onClick={onClickLogOut} className="text-danger"/>
        </div>
    </div>
  )
}
